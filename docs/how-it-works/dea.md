...
> **The default engine changed.** `stat_method` previously defaulted to `welch` (permutation); it now defaults to `limma`. [`run_de()`](../reference/tl/run_de.md) emits a one-time notice when limma runs — pass `stat_method` explicitly to state your choice.

The engine sets the defaults for two things that used to be manual knobs:
...
- **The q-value's correction defaults to the engine, and is selectable with `p_adjust`.** By default (`p_adjust="auto"`) the permutation engines report an empirical (permutation) FDR and limma reports Benjamini-Hochberg q-values. Pass any R `p.adjust` / limma `adjust.method` name — `"bh"`, `"by"`, `"holm"`, `"hochberg"`, `"hommel"`, `"bonferroni"` (case-insensitive) — to choose the correction; both engines apply it identically to their p-values (numerically matched to R's `p.adjust`). `"empirical"` is permutation-only, since limma has no null distribution to build it from. There is no "no correction" option — both engines always return `p_value` and `q_value`, so for uncorrected significance read the `p_value` column. (The old `fdr` argument was replaced by `p_adjust`.)
...
This is the **default** engine — `stat_method="limma"`, or simply omit the argument. It borrows information across features through an empirical-Bayes shrinkage of the per-feature variance, which stabilizes each variance estimate and recovers power at small sample sizes. `msmu`'s limma is a thin layer over [inmoose](https://inmoose.readthedocs.io/) (a Python port of Smyth's limma, validated numerically against R limma) and reports Benjamini-Hochberg q-values by default (choose another correction with `p_adjust`; see above). The moderated-t method is due to [Smyth, Statistical Applications in Genetics and Molecular Biology, 2004](https://doi.org/10.2202/1544-6115.1027), and the limma framework is described in [Ritchie et al., Nucleic Acids Research, 2015](https://doi.org/10.1093/nar/gkv007).
