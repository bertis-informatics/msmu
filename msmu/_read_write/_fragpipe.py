...
class FragPipeReader(SearchResultReader):
    def __init__(self, evidence_file: str | Path, label: Literal["tmt", "label_free"] | None = None) -> None:
        super().__init__()
    ...
    def _make_needed_columns_for_evidence(self, evidence_df: pd.DataFrame) -> pd.DataFrame:
        ...
        evidence_df.loc[evidence_df["peptide"].isna(), "peptide"] = evidence_df.loc[
            evidence_df["peptide"].isna(), "Peptide"
        ]
